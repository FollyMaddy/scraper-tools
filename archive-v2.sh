...
case "$LINE" in
  *cia* |*rar/* |*zip* | *iso*)
  ...
  wgetinfo=$(cat $startworkdir/$RP_map_structure/$website/$maplist/00wgetlogs/$MAPORFILE.info 2>&-)
  #echo $wgetinfo
  if [[ "$wgetinfo" != *"416 Requested Range Not Satisfiable"* ]] && [[ "$wgetinfo" != *"100%"* ]];then
  
  #time added for good continue -T2 => -T3
  echo Getting $MAPORFILE
  ...
  #test \r
  wget -T3 -t0 -c -w1 https://$website/$websitemap/$maplist/$MAPORFILE 2>&1 | tee $startworkdir/$RP_map_structure/$website/$maplist/00wgetlogs/$MAPORFILE.info | while read line; do if [[ $line != *".........."* ]] && [[ $line != *"100%"* ]]; then echo $line ; else sleep 0.0; echo -ne '                                                                                                                        \r'; echo -ne $line '\r'; fi; done


  echo
  else
  echo Skipping $MAPORFILE
